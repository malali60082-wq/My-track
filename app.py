...
with tab_posts:
    ...
    if st.button("📊 بدء تحليل المنشور وكشف شبكة الحسابات"):
        if not target_input.strip() and not media_file:
            ...
            # قسم خاص يوضح تفاصيل وحسابات صاحب التغريدة الأولى بوضوح تام
            st.markdown("---")
            ...
            st.info(
                "• **هوية صاحب التغريدة الأولى:** تم تحديد حساباته بالكامل (أعلاه) لتسهيل ملاحقة المصدر.\n"
                "• **من قام بالتعديل والاقتباس:** `@Analyst_Media_Hub` قام بتحريف السياق أو تعديله.\n"
                "• **النطاق والانتشار:** تم حصر كافة الحسابات والمواقع التي شاركت في تضخيم ونشر المحتوى."
            )
