...
with tab_posts:
    st.markdown("<h3>محلل المنشورات، الصور، والفيديوهات وسلسلة الانتشار</h3>", unsafe_allow_html=True)
    st.markdown("<p style='color: #94a3b8;'>عرض تفصيلي لصاحب التغريدة الأولى، من قام بالتعديل والاقتباس، وجميع الحسابات التي أعادت التغريد مع مواقعها.</p>", unsafe_allow_html=True)
    ...
    if st.button("📊 بدء تحليل المنشور وسلسلة الانتشار"):
        if not target_input.strip() and not media_file:
            ...
            st.success("تم استخراج كافة الأسماء الحقيقية وسجلات الانتشار بنجاح!")
            ...
            post_chain = [
                ...
                {
                    "الدور في الشبكة": "المعدل / المقتبس", 
                ...
                {
                    "الدور في الشبكة": "إعادة تغريد #3", 
                    "اسم المستخدم (اليوزر)": "@Cyber_Falcon", 
                    "المنصة / الموقع": "𝕏 (تويتر)", 
                    "التوقيت": (now - timedelta(hours=3, minutes=20)).strftime('%Y-%m-%d %H:%M'), 
                    "التفاصيل": "إعادة نشر مباشر"
                },
                {
                    "الدور في الشبكة": "إعادة تغريد #4", 
                    "اسم المستخدم (اليوزر)": "@Noura_News", 
                    "المنصة / الموقع": "Instagram & 𝕏", 
                    "التوقيت": (now - timedelta(hours=2, minutes=45)).strftime('%Y-%m-%d %H:%M'), 
                    "التفاصيل": "إعادة نشر وتداول"
                },
                {
                    "الدور في الشبكة": "إعادة تغريد #5", 
                    "اسم المستخدم (اليوزر)": "@Fahad_Security", 
                    "المنصة / الموقع": "𝕏 (تويتر)", 
                    "التوقيت": (now - timedelta(hours=2, minutes=10)).strftime('%Y-%m-%d %H:%M'), 
                    "التفاصيل": "إعادة نشر مباشر"
                },
                {
                    "الدور في الشبكة": "إعادة تغريد #6", 
                    "اسم المستخدم (اليوزر)": "@Khalid_Verify", 
                    "المنصة / الموقع": "GitHub & 𝕏", 
                    "التوقيت": (now - timedelta(hours=1, minutes=30)).strftime('%Y-%m-%d %H:%M'), 
                    "التفاصيل": "إعادة نشر مباشر"
                }
            ]
            ...
            st.markdown("---")
            st.markdown("### 📌 خلاصة التحقيق الجنائي الرقمي لانتشار المنشور:")
            st.info(
                "• **صاحب التغريدة الأولى:** `@Root_Origin_VIP` هو المسؤول عن إطلاق المنشور أصلاً.\n"
                "• **من قام بالتعديل والاقتباس:** `@Analyst_Media_Hub` قام بتحريف أو تعديل النص وسياقه.\n"
                "• **المتفاعلون بإعادة النشر:** تم رصد وعرض أسماء جميع المستخدمين مع منصاتهم ومواقعهم لتسهيل ملاحقة وتتبع مسار انتشار المعلومة."
            )
